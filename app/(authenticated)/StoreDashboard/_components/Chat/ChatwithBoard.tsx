...
console.log('Message:', JSON.stringify(messages, null, 2));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/StoreDashboard/chat", {
        method: "POST",
  ...
  prev.map((msg) =>
    msg.id === assistantMsgId
      ? {
          ...
          table: result.table ?? null,   // <-- ADD THIS LINE!
        }
      : msg
  )
);
  ...
  const handleFavorite = async (message: any) => {
    ...
    const favoriteResponse = await addFavorite(message.id, newFavoriteStatus);
    if (!favoriteResponse?.success) {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === message.id ? { ...msg, favorite: message.favorite } : msg
        )
      );
    } else {
      toast.success(
  ...
  return (
    <div className="mt-5 flex w-full justify-center h-screen ">
      <div className="flex flex-col justify-center items-center w-full h-full">
              ...
              {messages.map((message, index) => (
                ...
                >
                  ...
                  <div className="flex w-[680px] flex-col gap-2">
                    <div
                      className={`rounded-t-md break-words break-all overflow-y-auto rounded-l-lg p-3 ${message.role === "user" ? "bg-muted" : "bg-muted"
                        }`}
                    >
                    ...
                    {message.table && message.table.columns && message.table.data && (
                      <div className="overflow-auto mt-4 rounded-md border border-gray-300">
                        <table className="w-full text-sm table-auto border-collapse border border-gray-300">
                          <thead className="bg-gray-100">
                            <tr>
                              {message.table.columns.map((col: string, idx: number) => (
                                <th
                                  key={idx}
                          ...
                          <tbody>
                            {message.table.data.map((row: any[], rowIndex: number) => (
                                ...
                                {row.map((cell, cellIndex) => (
                                  ...
                                  >
                                    {typeof cell === "object" && cell !== null
                                      ? cell.display || cell.value || JSON.stringify(cell)
                                      : cell}
                                  </td>
                                ))}
                              </tr>
                    ...
                    {message.role === "assistant" && (
                      <div className="flex md:ml-3 items-center gap-5">
                        <div className="flex items-center gap-5">
                          ...
                          <Star
                            className={`w-5 h-5 cursor-pointer text-muted-foreground ${message.favorite ? "fill-primary text-primary" : ""
                              }`}
                        ...
                        <div className="flex items-center gap-5 justify-end w-full">
                          <ThumbsUp
                            ...
                            className={`w-5 h-5 ${message.isLike === true &&
                              "fill-primary text-primary"
                              } cursor-pointer text-muted-foreground`}
                          />
                          <ThumbsDown
                            onClick={() => handleLike(message, false)}
                            className={`w-5 h-5 ${message.isLike === false &&
                              "fill-primary text-primary"
                              } cursor-pointer text-muted-foreground`}
                          />
                        </div>
        ...
        >
          <div className="rounded-2xl flex items-center p-3.5 border shadow-sm bg-background">
            <Input
              placeholder="Enter the sales Prompt here "
              value={prompt}
